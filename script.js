...
// ===== UTIL =====
// ===== PERBAIKAN #1: Format Rupiah Input =====
function formatRupiahInput(input) {
    ...
    // Kembalikan posisi kursor (disesuaikan)
...
// ===== PERBAIKAN #2: Hanya Angka (izinkan Backspace, Delete, Arrow, Tab) =====
function hanyaAngka(event) {
    const allowedKeys = [8, 46, 37, 38, 39, 40, 9, 36, 35, 13]; 
    // 8=Backspace, 46=Delete, 37-40=Arrow, 9=Tab, 36=Home, 35=End, 13=Enter
    if (allowedKeys.includes(event.keyCode)) return true;
    return event.keyCode >= 48 && event.keyCode <= 57;
}

// ===== PERBAIKAN #3: Ambil Nilai Nominal (regex diperbaiki) =====
function getNominalValue() {
    let value = document.getElementById('txAmount').value;
    return parseInt(value.replace(/\./g, '')) || 0; // ✅ Titik di-escape
}
...
function renderTopList() {
  ...
  document.getElementById('topList').innerHTML = sorted.length ? sorted.map(([k,v],i) => {
    ...
    return `<div class="top-item">
      ...
      <div class="top-info"><div class="name">${m.label}</div><div class="count">${v.count} transaksi</div></div>
      <div class="top-value">${fmt(v.omzet)}</div>
    </div>`;
...
function openTx(key) {
  ...
  if (item.itemType === 'product') {
    ...
    document.getElementById('txSellPrice').value = item.sellPrice;
    document.getElementById('txQty').value = 1;
    document.getElementById('labelAmount').textContent = 'Total Omzet (Qty × Harga Jual)';
    document.getElementById('labelTotal').textContent = 'Total Bayar Pelanggan';
    document.getElementById('rowFee').style.display = 'none';
    document.getElementById('rowFeeSummary').style.display = 'none';
    document.getElementById('rowProfit').style.display = 'flex';
    // 🆕 Info stok produk
    const currentStock = state.stock[item.key] || 0;
    const stockInfo = document.getElementById('productStockInfo');
    if (currentStock === 0) {
      stockInfo.innerHTML = `<span style="color:var(--danger);font-weight:700;">⛔ STOK HABIS! Tidak bisa menjual.</span>`;
      document.getElementById('btnProses').disabled = true;
      document.getElementById('btnProses').style.opacity = '0.5';
      document.getElementById('btnProses').textContent = '⛔ Stok Habis';
    } else if (currentStock < 3) {
      stockInfo.innerHTML = `<span style="color:var(--warning);font-weight:700;">⚠️ Stok tersisa ${currentStock} unit (minimal 3)</span>`;
    } else {
      stockInfo.innerHTML = `<span style="color:var(--success);">✅ Stok tersedia: <b>${currentStock} unit</b></span>`;
    }
    document.getElementById('modalInfoText').textContent = `💵 Cash akan bertambah ${fmt(item.sellPrice)} × qty`;
  } else {
...
function calcTotal() {
  ...
  if (itemType === 'product') {
    const sellPrice = +document.getElementById('txSellPrice').value || 0;
    ...
    document.getElementById('sumProfit').textContent = '+' + fmt(revenue);
    document.getElementById('sumTotal').textContent = fmt(amount);
  } else {
...
function submitTx(e) {
  ...
  if (itemType === 'product') {
    sellPrice = +document.getElementById('txSellPrice').value || 0;
    qty = +document.getElementById('txQty').value || 1;
  ...
  renderLowStockAlert();
  showReceipt(tx);
  // 🆕 NOTIFIKASI STOK RENDAH SETELAH TRANSAKSI
  if (itemType === 'product') {
...
function showReceipt(tx) {
  ...
  if (tx.type === 'add_modal') {
    ...
    if (tx.itemType === 'product') {
      extraRows = `
        <div class="row"><span>Produk</span><span>${item.label}</span></div>
...
function renderHistory() {
  ...
  document.getElementById('historyTable').innerHTML = txs.length ? txs.map(t => {
    const item = getItem(t.type);
    if (!item && t.type !== 'add_modal') return '';
    const tipeBadge = t.type === 'add_modal'
      ? `<span class="badge badge-success">Tambah Modal</span>`
      : (item.itemType === 'product' ? `<span class="badge badge-product">Produk</span>` : `<span class="badge badge-service">Layanan</span>`);
    let dampak = '-';
    if (t.type === 'add_modal') {
...
function renderReport() {
  ...
  document.getElementById('reportSummary').innerHTML = `
    <div class="report-card"><div class="label">Total Transaksi</div><div class="value">${monthTx.length}</div></div>
...
function loadSettings() {
  ...
  document.getElementById('setModalBrmo').value = state.settings.modalBrimo || 0;
  document.getElementById('setModalCash').value = state.settings.modalCash || 0;
...
// ===== 🆕 RESET ALL DATA (DENGAN KONFIRMASI KUAT) =====
function resetAllData() {
  // Step 1: Peringatan pertama
  if (!confirm('⚠️ PERINGATAN KERAS!\n\nAnda akan menghapus SEMUA data transaksi, pengaturan, dan stok produk.\n\nTindakan ini TIDAK DAPAT dibatalkan!\n\nLanjutkan?')) {
    return;
  }
  // Step 2: Konfirmasi kedua dengan input
...
function saveEditTx(e) {
  ...
  if (tx.type === 'add_modal') {
    ...
    // 🆕 Sesuaikan stok saat edit qty produk
